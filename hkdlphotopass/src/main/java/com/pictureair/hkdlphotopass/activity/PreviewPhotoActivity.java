...
import com.pictureworks.android.GalleryWidget.GalleryViewPager;
import com.pictureworks.android.GalleryWidget.UrlPagerAdapter;
...
import com.pictureworks.android.util.AESKeyHelper;
...
import com.pictureworks.android.util.HttpCallback;
import com.pictureworks.android.util.HttpUtil1;
...
import com.pictureworks.android.util.UmengUtil;
...
//                intent.putExtra("isBack", "1");//取消付款后是否回到当前页面
            ...
            case API1.GET_GOODS_SUCCESS:
                ...
                if (goodsInfoJson != null && goodsInfoJson.getGoods().size() > 0) {
                    ...
                    API1.addToCart(MyApplication.getTokenId(), pppGoodsInfo.getGoodsKey(), 1, true, null, previewPhotoHandler);
                    //将数据保存到缓存中
...
//                judgeBuyOnePhoto(); // 根据需求， 取消了购买的流程 的同步流程
            ...
            case GET_LOCATION_AD:
                ...
                if (myApplication.isGetADLocationSuccess()) {
                    ...
                    //从网络获取
                    API1.getADLocations(MyApplication.getTokenId(), previewPhotoHandler);
                }
                if (dialog.isShowing()) {
...
//                    }
                    //将 tabname 存入sp
                    Editor editor1 = sharedPreferences.edit();  //设置需要刷新
...
//        judge();//判断 照片是否购买，并弹出相应的tips
    ...
    private void init() {
        ...
        sharePop = new SharePop(this, MyApplication.getTokenId());
        matrix = new Matrix();
...
//                            downLoadPhotos();
            ...
            case R.id.buynow:
                ...
                API1.buyPhoto(MyApplication.getTokenId(), MyApplication.getInstance().getLanguageType(), photoInfo.photoId, previewPhotoHandler);
                dia.dismiss();
                break;
            ...
            case R.id.use_ppp:
                if (AppUtil.getNetWorkType(PreviewPhotoActivity.this) == AppUtil.NETWORKTYPE_INVALID) { //判断网络情况。
                    ...
                    API1.getPPPsByShootDate(MyApplication.getTokenId(), previewPhotoHandler, photoInfo.shootTime);
                }
                break;
    ...
    public void getALlGoods() {
        ...
        if (goodsByACache != null && !goodsByACache.equals("")) {
            ...
            if (AppUtil.getNetWorkType(MyApplication.getInstance()) != 0) {
                API1.getGoods(MyApplication.getTokenId(), MyApplication.getInstance().getLanguageType(), previewPhotoHandler);
            } else {
                //提醒检查网络
