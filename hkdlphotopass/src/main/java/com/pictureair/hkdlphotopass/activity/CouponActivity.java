...
public class CouponActivity extends BaseActivity implements CouponViewInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        couponTool = new CouponTool(this, MyApplication.getTokenId(), MyApplication.getInstance().getLanguageType(), this);
        Intent intent = new Intent();
        intent.putExtra("type", "coupon");//只扫描coupon
        dealCodeUtil = new DealCodeUtil(this, intent, false, MyApplication.getInstance().getLanguageType(), myHandler);
        initViews();
            ...
            switch (msg.what) {
                case DialogInterface.BUTTON_POSITIVE://点击确定，添加code
                    if (msg.obj.toString().length() == 0) {
                        ...
                        showProgressBar();
                        dealCodeUtil.startDealCode(msg.obj.toString(), Common.APP_TYPE_HKDLPP);
                    }
                    break;
