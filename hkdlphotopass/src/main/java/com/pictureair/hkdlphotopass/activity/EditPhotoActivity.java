...
import com.pictureworks.android.editPhoto.EditPhotoUtil;
import com.pictureworks.android.editPhoto.Matrix3;
import com.pictureworks.android.editPhoto.StickerItem;
import com.pictureworks.android.editPhoto.StickerView;
import com.pictureworks.android.entity.DiscoverLocationItemInfo;
import com.pictureworks.android.entity.EditPhotoInfo;
import com.pictureworks.android.entity.FrameOrStikerInfo;
import com.pictureworks.android.entity.PhotoInfo;
import com.pictureworks.android.entity.StikerInfo;
import com.pictureworks.android.filter.Amaro;
import com.pictureworks.android.filter.BeautifyFilter;
import com.pictureworks.android.filter.BlurFilter;
import com.pictureworks.android.filter.EarlyBird;
import com.pictureworks.android.filter.Filter;
import com.pictureworks.android.filter.HDRFilter;
import com.pictureworks.android.filter.LomoFi;
import com.pictureworks.android.filter.LomoFilter;
import com.pictureworks.android.filter.NormalFilter;
import com.pictureworks.android.filter.OldFilter;
...
import com.pictureworks.android.util.LocationUtil;
...
import com.pictureworks.android.widget.HorizontalListView;
...
//			public void run() {
		...
		initData();
		//开始从网络获取最新数据
		API1.getLastContent(appPreferences.getString(Common.GET_LAST_CONTENT_TIME, null), MyApplication.getTokenId(), editPhotoHandler);
//			};
//		}.start();
...
//		frameInfos.add(frameInfo);
	...
	protected void onResume() {
		...
		if (locationItemInfos.size() == 0) {//说明不存在，需要获取所有的location地点信息
			locationItemInfos.clear();
			locationItemInfos.addAll(AppUtil.getLocation(this, ACache.get(this).getAsString(Common.LOCATION_INFO), false));
...
//			
				...
				onEditStates();
				eidtAdapter = new EditActivityAdapter(EditPhotoActivity.this,mainBitmap, new ArrayList<String>(),1, frameInfos, Common.APP_TYPE_HKDLPP, editPhotoHandler);
...
//			});
			...
			case R.id.edit_filter:
				...
				eidtAdapter = new EditActivityAdapter(EditPhotoActivity.this, mainBitmap, filterPathList, 2, new ArrayList<FrameOrStikerInfo>(), Common.APP_TYPE_HKDLPP, editPhotoHandler);
				top_HorizontalListView.setAdapter(eidtAdapter);
			...
			case R.id.edit_accessory:
				...
				eidtAdapter = new EditActivityAdapter(EditPhotoActivity.this,mainBitmap, new ArrayList<String>(),3, stikerInfos, Common.APP_TYPE_HKDLPP, editPhotoHandler);
				top_HorizontalListView.setAdapter(eidtAdapter);
				top_HorizontalListView.setOnItemClickListener(new OnItemClickListener() {
