            ...
            switch (msg.what) {
                ...
                case API.GET_ORDER_NO_SUCCESS:
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        String s = String.valueOf(list.get(i).getQty());
...
//                    params.put(Common.ITEM, cartItem);
    ...
    private void initview() {
        ...
        list = (ArrayList<CartItemInfo1>) getIntent().getSerializableExtra("orderinfo");//获取照片路径
        ...
        for (int i = 0; i < list.size(); i++) {
            if (i == 0) {
                nameString = list.get(i).getProductName();
                introduceString = list.get(i).getDescription();
            } else {
                nameString += "," + list.get(i).getProductName();
                introduceString += "," + list.get(i).getDescription();
            }
            totalprice += list.get(i).getPrice() * list.get(i).getQty();

            list.get(i).setShowPhotos(0);
            cartItemId = new JSONObject();
            try {
                cartItemId.put("cartItemId", list.get(i).getCartId());
            } catch (JSONException e) {
                e.printStackTrace();
            }
