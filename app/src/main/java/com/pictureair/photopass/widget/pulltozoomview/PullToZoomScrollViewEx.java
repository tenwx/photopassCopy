...
public class PullToZoomScrollViewEx extends PullToZoomBase<ScrollView> {
        ...
        ((InternalScrollView) mRootView).setOnScrollViewChangedListener(new OnScrollViewChangedListener() {
            ...
            public void onInternalScrollChanged(int left, int top, int oldLeft, int oldTop) {
                if (isPullToZoomEnabled() && isParallax()) {
                    PictureAirLog.d(TAG, "onScrollChanged --> getScrollY() = " + mRootView.getScrollY());
                    float f = mHeaderHeight - mHeaderContainer.getBottom() + mRootView.getScrollY();
                    PictureAirLog.d(TAG, "onScrollChanged --> f = " + f);
    ...
    protected void pullHeaderToZoom(int newScrollValue) {
        PictureAirLog.d(TAG, "pullHeaderToZoom --> newScrollValue = " + newScrollValue);
        PictureAirLog.d(TAG, "pullHeaderToZoom --> mHeaderHeight = " + mHeaderHeight);
        ...
        if (isCustomHeaderHeight) {
            updateZoomView(Math.abs(newScrollValue) + mHeaderHeight - offset);
        }
    }
    ...
    private void updateZoomView(int height) {
        ...
        ImageView imageView = (ImageView) mZoomView.findViewById(R.id.code_pic);
        ViewGroup.LayoutParams layoutParams = imageView.getLayoutParams();
        layoutParams.width = zoomLayoutParams.height / 2;
    ...
    protected void smoothScrollToTop() {
        PictureAirLog.d(TAG, "smoothScrollToTop --> ");
    ...
    public void setHeaderLayoutParams(LinearLayout.LayoutParams layoutParams) {
        if (mHeaderContainer != null) {
            ...
            updateZoomView(mHeaderHeight - offset);
        }
    }
        ...
        public void run() {
            if (mZoomView != null) {
                ...
                if ((!mIsFinished) && (mScale > 1.0D)) {
                    ...
                    PictureAirLog.d(TAG, "ScalingRunnable --> f2 = " + f2);
                    if (f2 > 1.0F) {
                        localLayoutParams.height = ((int) (f2 * mHeaderHeight));
                        mHeaderContainer.setLayoutParams(localLayoutParams);
                        if (isCustomHeaderHeight) {
                            updateZoomView(((int) (f2 * mHeaderHeight)) - offset);
                        }
                        post(this);
                        return;
                    }
