...
public class CheckUpdateManager {
    ...
    private String downloadURL, forceUpdate, currentLanguage, tokenId, appType;
...
//        pictureWorksDialog.show();
     ...
     */
    public CheckUpdateManager(Context context, String currentLanguage, String tokenId, String appType, View parent, DownloadListener downloadListener) {
        ...
        dbDAO = new PictureAirDbManager(context, PWJniUtil.getSqlCipherKey(appType));
    }
    ...
    public void startCheck() {
        ...
        if (tokenId == null) {
            baseCheckUpdate.getTokenId(context, appType, handler);
        } else {
            checkApk();
        }
    }
    ...
    private void checkApk(){
        baseCheckUpdate.checkUpdate(tokenId, context, handler, deviceInfos.get(1), currentLanguage);
    }
    ...
    public void dialogButtonPositive() {
        ...
        if (downloadAPKFile.exists() && !dbDAO.isExistsThread()) {//文件已经存在
            ...
            downloadListener.download(downloadURL);
