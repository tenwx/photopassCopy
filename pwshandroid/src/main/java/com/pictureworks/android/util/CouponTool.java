...
import com.pictureworks.android.widget.CouponViewInterface;
...
public class CouponTool {
    ...
    private String tokenId, language;
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case API1.INSERT_COUPON_FAILED://添加一张优惠卷失败
                    int id = ReflectionUtil.getStringId(context, msg.arg1);
                    if (couponView != null) {
                        couponView.goneProgressBar();
                        couponView.fail(id);
                    }
    ...
    public CouponTool(Context context, String tokenId, String language, CouponViewInterface couponView) {
    ...
    public void queryCouponOrderPage(JSONArray cartItemIds) {
        ...
        API1.getCartItemCoupons(tokenId, mHandler, cartItemIds);
    }
    ...
    public void queryCouponMePage() {
        ...
        API1.getCoupons(tokenId, mHandler);//从me中进来的
        /**
         * test
         */
...
//        couponView.goneProgressBar();
    ...
    public void insertCoupon(String cpCode) {
        ...
        if (whatPege.equals(ACTIVITY_ORDER)) {
            API1.addCoupons(tokenId, mHandler, cpCode, cartItemIds);
        } else if (whatPege.equals(ACTIVITY_ME)) {
            API1.addCoupons(tokenId, mHandler, cpCode, null);
        } else {
            couponView.goneProgressBar();
    ...
    private void getApiReturnDatas(JSONObject jsonObject) {
        ...
        if (whatPege.equals(ACTIVITY_ME)){//从me中进来的
            mDatas = JsonUtil.getCouponListFromJson2(jsonObject, language);
        }else{//从订单进来
            mDatas = JsonUtil.getCouponListFromJson(jsonObject, language);
        }
        couponView.goneProgressBar();
    ...
    private boolean getNetwork() {
        return AppUtil.NETWORKTYPE_INVALID != AppUtil.getNetWorkType(context);
    }
