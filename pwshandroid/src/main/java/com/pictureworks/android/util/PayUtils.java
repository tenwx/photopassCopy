...
import com.pictureworks.android.alipay.PayResult;
...
public class PayUtils {
    ...
    public static final int ASYNC_PAY_SUCCESS = 6;
    ...
    public void wxDealResult(int resultCode) {
        switch (resultCode) {
            case 0:// 微信支付成功
                handler.sendEmptyMessage(RQF_SUCCESS);
                break;

            case -1:// 微信支付取消
            case -3:
                handler.sendEmptyMessage(RQF_CANCEL);
                break;

            case -2:// 微信支付失败
                handler.sendEmptyMessage(RQF_UNSUCCESS);
                break;
