...
public class DealCodeUtil {
			...
			switch (msg.what) {
				...
				case API1.ADD_PPP_CODE_TO_USER_SUCCESS://绑定ppp成功
					...
					if (msg.obj == null) {//ppp
						...
						CouponInfo couponInfo = JsonUtil.getCouponInfo(((JSONObject) msg.obj).getJSONObject("PPP"), language);
						//将对象放入bundle3中
						bundle3.putInt("status", 3);
	...
	public DealCodeUtil(Context context, Intent intent, boolean isInputAct, String language, Handler handler) {
	 ...
	 * @param code
	 */
	public void startDealCode(final String code, String appType){
		this.code = code;
		this.appType = appType;
		API1.checkCodeAvailable(code, AESKeyHelper.decryptString(sharedPreferences.getString(Common.USERINFO_TOKENID, ""), PWJniUtil.getAESKey(appType)), handler2);
	}

	private void getInfo(String code, final String type){
		RequestParams params = new RequestParams();
		PictureAirLog.out("scan result=" + code + ">>" + type);
		params.put(Common.USERINFO_TOKENID, AESKeyHelper.decryptString(sharedPreferences.getString(Common.USERINFO_TOKENID, ""), PWJniUtil.getAESKey(appType)));
		...
		if ("pp".equals(type)) {
			if (null != needBind && "false".equals(needBind)) {//如果是通过pp界面扫描的时候，此处不需要绑定pp到用户
				...
				API1.bindPPsToPPP(AESKeyHelper.decryptString(sharedPreferences.getString(Common.USERINFO_TOKENID, null), PWJniUtil.getAESKey(appType)), pps, bindData, pppId, handler2);
