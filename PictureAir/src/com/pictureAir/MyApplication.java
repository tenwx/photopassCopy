...
//	private HashMap<String, Boolean> isBuyingPhotoFromAlbumHashMap = new HashMap<String, Boolean>();//记录是否从相册页面购买的单张照片（此处的相册有两处，第一次ViewOrSelectActivity页面，第二处是LocationPhotosAct）
...
//		mLocationClient.start();
	...
	private static void initImageLoader(Context context) {
		//		File parent = new File(Environment.getExternalStorageDirectory().getPath() + "/pictureAir/cache/image2/");
		//		if (!parent.exists()) {
		//			parent.mkdirs();
		//		}
//		File cacheDir = StorageUtils.getOwnCacheDirectory(context, "/pictureAir/cache/images/");
		File cacheDir = StorageUtils.getCacheDirectory(context);;
		// displayImage(...) call if no options will be passed to this method
		DisplayImageOptions defaultOptions = new DisplayImageOptions.Builder()
		.cacheInMemory(true).cacheOnDisk(true).
		considerExifParams(true).//考虑图片的exif属性
		//		showStubImage(imageRes)//图片下载期间显示的图片
		//		.showImageForEmptyUri(R.drawable.ic_empty)  // 设置图片Uri为空或是错误的时候显示的图片  
		showImageOnLoading(R.drawable.ic_loading).showImageOnFail(R.drawable.ic_failed).build();
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(context).memoryCacheExtraOptions(1000, 1000)
				// max width, max height，即保存的每个缓存文件的最大长宽
				.threadPoolSize(4)
				// 线程池内加载的数量
				.threadPriority(Thread.NORM_PRIORITY - 2).denyCacheImageMultipleSizesInMemory().memoryCacheSize(2 * 1024 * 1024)
...
//LimitedAgeDiskCache (不限制缓存大小，但是设置缓存时间，到期后删除)
				.diskCacheSize(50 * 1024 * 1024)//50m本地缓存
				.defaultDisplayImageOptions(defaultOptions).imageDownloader(new BaseImageDownloader(context, 5 * 1000, 30 * 1000)) // connectTimeout
				.build();// 开始构建
