...
public class PreviewPhotoActivity extends BaseActivity implements
		OnClickListener {
	// 工具条
	...
	private void init() {
		...
		System.out.println("photoid ===== "
				+ getIntent().getStringExtra("photoId"));

		// 获取intent传递过来的信息
		photolist = (ArrayList<PhotoInfo>) getIntent().getSerializableExtra(
				"photos");// 获取图片路径list
		targetphotolist = (ArrayList<PhotoInfo>) getIntent()
				.getSerializableExtra("targetphotos");
		...
		// }else{
		// 本地图片
		// FilePagerAdapter pagerAdapter = new FilePagerAdapter(this,
		// photolist);
	...
	public void onClick(View v) {
		...
		case R.id.preview_edit:// 编辑
			...
			if (isEdited) {// 已经编辑过，取targetlist中的值
				...
				// targetphotolist.get(mViewPager.getCurrentItem()).photoPathOrURL);
				// intent2.putExtra("album", flag);
				// intent2.putExtra("id",
				// targetphotolist.get(mViewPager.getCurrentItem()).photoId);
				intent2.putExtra("photo",
						targetphotolist.get(mViewPager.getCurrentItem()));
			} else {// 没有编辑，取正常的值
				// intent2.putExtra("photoURL",
				// photolist.get(mViewPager.getCurrentItem()).photoPathOrURL);
				// intent2.putExtra("album",
				// getIntent().getStringExtra("flag"));
				// intent2.putExtra("id",
				// photolist.get(mViewPager.getCurrentItem()).photoId);
				intent2.putExtra("photo",
						photolist.get(mViewPager.getCurrentItem()));
			}
		...
		case R.id.preview_share:
			System.out
					.println("start share="
							+ photolist.get(mViewPager.getCurrentItem()).photoPathOrURL);
			if (isEdited) {// 编辑后
				sharePop.setshareinfo(
						targetphotolist.get(mViewPager.getCurrentItem()).photoPathOrURL,
						null, null, "local", handler);
			} else {// 编辑前
					// 判断图片是本地还是网路图片
		...
		case R.id.preview_download:// 下载,如果不是pp的照片，提示不需要下载，如果是pp的照片，并且没有支付，提示购买，如果已经购买，如果没有下载，则下载，否则提示已经下载
			if (isEdited) {// 编辑后
				newToast.setTextAndShow(R.string.neednotdownload,
		...
		case R.id.preview_makegift:
			...
			if (isEdited) {// 已经被编辑过，那么取得是targetList中的值
				intent.putExtra("selectPhoto",
						targetphotolist.get(mViewPager.getCurrentItem()));
			} else {// 没有编辑过，直接获取之前的值
				// if (Common.ALBUM_PHOTOPASS.equals(flag)) {
				// intent.putExtra("thumbnailURL",
				// photolist.get(mViewPager.getCurrentItem()).photoThumbnail_512);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 1) {
			if (resultCode == 11) {
				...
				selectPhotoItemInfo.photoPathOrURL = data
						.getStringExtra("photoUrl");
				// 2.将新图片插入到targetList中
				targetphotolist.add(0, selectPhotoItemInfo);
				// 3.修改viewPager中的值为targetList
				mViewPager
						.setAdapter(new UrlPagerAdapter(this, targetphotolist));
