...
//	private Handler mHandler;
	private ImageButton wechat, qqzone, sina, facebook;
...
//	private PlatformActionListener callback;
	...
	public void initPopupWindow() {
		...
		facebook = (ImageButton)defaultView.findViewById(R.id.facebook);
...
//		shareParams.url = "http://www.pictureair.com";//share_webpage的时候需要这个参数
	...
	public void qzoneShare(Context context, String imagePath, String imageUrl, String type){
		...
		shareParams.text = context.getResources().getString(R.string.qq_zone_share);
		if ("local".equals(type)) {//本地图片
			shareParams.imagePath = imagePath;
		}else if ("online".equals(type)) {//网络图片
			shareParams.imageUrl = imageUrl;
...
//			shareParams.text = imageUrl;
	 ...
	 * @param commend 文本评论
	 */
	public void facebookShare(Context context, String imagePath, String imageUrl, String type){
		System.out.println("sharing info "+ imagePath+"_"+imageUrl+"_"+type);
...
//			newToast.setTextAndShow("This function doesn't open", Common.TOAST_SHORT_TIME);
		...
		case R.id.facebook:
			facebookShare(context, imagePath, imageUrl, type);
			break;
...
//			Toast.makeText(context, text, Toast.LENGTH_SHORT).show();
	...
	private void showNotification(long cancelTime, String text) {
		try {
			...
			Notification notification = new Notification(R.drawable.pp_icon, text, when);
			PendingIntent pi = PendingIntent.getActivity(app, 0, new Intent(), 0);
			notification.setLatestEventInfo(app, context.getString(R.string.app_name), text, pi);
