...
public class ViewPhotoActivity extends BaseActivity implements OnClickListener {
	...
	private TextView selectDisAll, selectAll, selectMakeGift, selectShare,
			selectDelete;
	...
	private boolean isSelectAll = false;// 全选标记
	...
	private void initview() {
		...
		viewPhotoGridViewAdapter = new ViewPhotoGridViewAdapter(this,
				magicArrayList);
	...
	// 获取Magic的照片
	private void ScanPhotos(ArrayList<PhotoInfo> arrayList, String filePath,
			String albumName) {
		...
		for (int i = 0; i < files.length; i++) {
			if (files[i].getName().endsWith(".JPG")
				...
				if (files[i].length() > 0) {// 扫描到文件
					...
					System.out.println("magic url is =====>"
					...
					// selectPhotoItemInfo.location =
					// getString(R.string.story_tab_magic);
	...
	public void onClick(View v) {
		...
		case R.id.select_makegift:// 制作礼物操作
			...
			if (photoURLlist.size() == 0) {// 没有图片
				newToast.setTextAndShow(R.string.select_photos,
			...
			if (photoURLlist.size() == 0) {// 没选择图片
				...
				if (photoURLlist.get(0).onLine == 1) {// 网络图片
					...
					if (photoURLlist.get(0).isPayed == 0) {// 未购买
						newToast.setTextAndShow(R.string.buythephoto,
								Common.TOAST_SHORT_TIME);
					} else {
						sharePop.setshareinfo(null,
								photoURLlist.get(0).photoPathOrURL, null,
								"online", mHandler);
						sharePop.showAtLocation(v, Gravity.BOTTOM
								| Gravity.CENTER_HORIZONTAL, 0, 0);
					}

				} else {
					System.out.println("本地图片");
					sharePop.setshareinfo(photoURLlist.get(0).photoPathOrURL,
							null, null, "local", mHandler);
					sharePop.showAtLocation(v, Gravity.BOTTOM
							| Gravity.CENTER_HORIZONTAL, 0, 0);
				}
			} else {// 选择超过1张
	...
	private class ScanPhotosThread extends Thread {
		...
		public void run() {
			...
			ScanPhotos(magicArrayList, Common.PHOTO_SAVE_PATH,
					Common.ALBUM_MAGIC);
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (position == 0) {
				...
				if (magicArrayList.get(position).isChecked == 1) {// 通过判断check的标记来确定是否响应选择事件还是预览事件
					...
					viewPhotoGridViewAdapter.refreshView(position,
							myGridView.getChildAt(position - visiblePos), 1);
					...
					System.out.println(position + "_"
							+ magicArrayList.get(position).photoPathOrURL);
					Intent intent = null;
					intent = new Intent(ViewPhotoActivity.this,
	...
	/**
	 * 删除列表中的项 1.遍历整个list 2.如果找到目标，将resultstring设为true 3.遍历结束之后，将找到的目标删除
	 * 4.更新button的数值 5.更新适配器
	 * 
	 * @param bundle
	 */
	private void deletefromlist(Bundle bundle) {
		...
		for (int i = 0; i < photoURLlist.size(); i++) {
			if (bundle.getString("pathOrUrl").equals(
					photoURLlist.get(i).photoPathOrURL)) {// 如果找到之后立刻删除，会对list的长度有影响，for循环会报错
				result = i;
				resultString = true;
			}
		}
	...
	/**
	 * 添加选择的图片到列表 1.遍历列表 2.如果找到目标，说明之前已经添加过，所以不需要任何操作 3.如果没有找到目标，添加到列表中
	 * 4.通知适配器更新
	 * 
	 * @param b
	 */
	private void addtolist(Bundle b) {
		for (int j = 0; j < photoURLlist.size(); j++) {
			if (b.getString("pathOrUrl").equals(
					photoURLlist.get(j).photoPathOrURL)) {
				result = j;
				resultString = true;
			}
		}
		if (resultString) {
			...
			System.out.println("photoURLList" + photoURLlist.size()
					+ "magicarraylist is " + magicArrayList.size());
	...
	private void doWork(int id) {
		...
		for (int i = 0; i < photoURLlist.size(); i++) {
			...
			System.out.println("需要删除的文件为" + photoURLlist.get(i).photoPathOrURL
					+ ",需要删除的索引值为" + photoURLlist.get(i).index);
			...
			getContentResolver().delete(Media.EXTERNAL_CONTENT_URI,
					Media.DATA + " like ?", params);

			// 删除图片在arraylist中对应的项
			System.out.println("删除前的列表长度为" + magicArrayList.size()
					+ ",需要删除的索引值----->" + photoURLlist.get(i).index.toString());
			System.out
					.println("arraylist需要移除的文件是"
							+ magicArrayList.get(Integer.parseInt(photoURLlist
									.get(i).index.toString())).photoPathOrURL);
			magicArrayList.remove(Integer.parseInt(photoURLlist.get(i).index
					.toString()));// 删除列表中的对应的信息
			...
			if (file.exists()) {
				System.out.println("开始删除文件"
						+ photoURLlist.get(i).photoPathOrURL);
				// 删除文件
				file.delete();
